...
#   - 13-feature obs: standard 12 + worker_mkt_share (anti-hoarding signal)
#
# Observation (13 features):
#   0  profit_signal       — tanh(profit/20k)           own profit level
#   1  profit_change       — tanh(dp/5k)                own profit trend
...
#  12  worker_mkt_share    — tanh((own_L - avg_L)/5)    workforce vs firm average
...
class ReformedFirmEnv(gym.Env):
    ...
    def __init__(self):
        self.rl_firm_id   = "F0"
        self.model        = LaborMarketModel(use_wage_gap_prob=True, rl_firm_id=self.rl_firm_id)
        ...
        # 7 actions: hold / wage+300 / wage+100 / wage-100 / wage-300 / post_vacancy / fire_worker
    ...
    def observe(self):
        ...
        # Anti-hoarding: how much larger is this firm vs average?
        active_firms = model.active_firms()
        avg_workers  = float(np.mean([len(f.current_workers) for f in active_firms])) if active_firms else labor
        worker_mkt_share = float(np.tanh((labor - avg_workers) / 5.0))
