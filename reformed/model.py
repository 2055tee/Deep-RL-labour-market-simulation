# reformed/model.py
#
# Improved labor market model with RL support.
#
#   Option 3 — workers see max(3, N//4) firms per search  (was max(2, N//10))
#   Option 4 — wage-gap probability: employed workers below market wage search more
#              controlled by `use_wage_gap_prob` flag
#              (True during RL training; False for pure-heuristic baseline)
#   Option 5 — switching cost is utility-proportional (was raw THB)
#
# Market-quit: workers quit after MARKET_QUIT_PATIENCE months
#              below MARKET_QUIT_THRESHOLD * market_wage, even without a new job.
#              This is the primary structural fix for RL worker hoarding.
#
# RL support: set rl_firm_id to the index of the RL firm.
#             That firm's adjust_employment_step is skipped; it acts via rl_action.
#
# Firm replacement: when a firm exits, a new one is spawned to keep N_firms constant.
...
MAX_VACANCIES           = 5     # cap on RL firm vacancies
...
class Worker(Agent):
    ...
    def __init__(self, uid, model, hours_worked, non_labor_income, consumption_rate):
        ...
        self.job_search_prob   = 0.1          # base probability for employed search
        self.monthly_wage      = 0
        self.daily_wage        = 0
        self.steps             = 0
    ...
    def _firms_to_consider(self):
        """Option 3: see ~25% of active firms (was ~10%)."""
    ...
    def search_for_jobs(self, firms_to_consider):
        if self.employed:
            ...
            # Option 5: require a relative utility gain of 5%
    ...
    def job_search_step(self):
        ...
        if self.employed:
            # Option 4: boost search prob when own wage is below market
...
class Firm(Agent):
    """
    Heuristic firm — skips auto adjust_employment_step when acting as RL firm.
    RL firm controls hiring/firing via rl_decision() actions instead.
    """
    ...
    def adjust_employment_step(self):
        if not self.active:
            return
        # RL firm controls its own employment
    ...
    def rl_stage(self):
        if self.uid == self.model.rl_firm_id and self.active:
...
class LaborMarketModel(Model):
    ...
    def _spawn_replacement_firm(self):
        """Spawn a new heuristic firm to replace one that exited."""
