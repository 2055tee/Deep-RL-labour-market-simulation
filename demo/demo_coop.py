...
def ActionGrid(model):
    ...
    if N_RL_FIRMS == 1:
        axes = [axes]
    fig.suptitle("RL Firm Actions -- Last 72 months  [Cooperative]",
...
def ActionPieGrid(model):
    ...
    for idx, ax in enumerate(axes):
        ...
        ax.set_title(f"F{idx}  ({len(m.actions_each[idx])} steps)",
                     color=RL_FIRM_COLORS[idx], fontsize=9, fontweight="bold")
    fig.suptitle("Action Distribution [Cooperative]", color=TEXT, fontsize=11, fontweight="bold")
    plt.tight_layout(pad=0.3)
    solara.FigureMatplotlib(fig)
    plt.close(fig)
...
def RLWageSpreadChart(model):
    ...
    ax1.set_title("Individual RL Firm Profits  [Cooperative]", color=TEXT, fontsize=10, fontweight="bold")
    ...
    ax2.set_title("Wage Spread Among RL Firms  (should stay low in cooperative mode)", color=TEXT, fontsize=9)
    plt.tight_layout(pad=0.4)
    solara.FigureMatplotlib(fig)
    plt.close(fig)
