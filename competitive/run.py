# competitive/run.py  — evaluate 3 competitive RL firms vs 7 heuristic

import numpy as np
from sb3_contrib import MaskablePPO
from firm_env import CompFirmEnv, N_RL_FIRMS
...
def observe_firm(firm, prev_p, prev_w, step):
    ...
    if labor > 0:
        mpl      = firm.marginal_product_labor(firm.productivity, labor, firm.alpha)
        vmpl_gap_val = float(np.tanh((mpl * firm.output_price - firm.monthly_wage) / max(firm.monthly_wage, 1.0)))
    else:
        vmpl_gap_val = 1.0
    all_wages   = [f.monthly_wage for f in sim.firms]
    ...
    avg_prod    = float(np.mean([f.productivity for f in sim.firms]))
    avg_cap     = float(np.mean([f.capital      for f in sim.firms]))
    ...
    obs = np.array([profit_signal, profit_change_signal, vmpl_gap_val, wage_vs_mkt,
...
for step in range(N_STEPS):
    ...
    profits_all = sorted([f.profit for f in sim.firms], reverse=True)

    print(SEP)
    print(f"  Step {step:>3}  |  employment: {employ_rate:.1%}  |  "
          f"avg RL profit: {avg_rl_p:,.0f}  |  avg heuristic profit: {avg_h_p:,.0f}")
    print(SEP)
    print(f"  {'Firm':<8}  {'Type':<9}  {'Action':<7}  {'Profit':>10}  {'Rank':>4}  "
    ...
    for i, f in enumerate(rl_firms):
        ...
        rank = profits_all.index(f.profit) + 1
        print(f"  {f'F{i} [RL]':<8}  {'RL':<9}  {ACTION_LABELS[firm_actions[f.uid]]:<7}  "
              f"{f.profit:>10,.0f}  {rank:>4}  {f.monthly_wage:>7,}  {len(f.current_workers):>7}  {f.vacancies:>3}  {gs}")
    ...
    print(f"  RL vs heuristic: {avg_rl_p - avg_h_p:>+10,.0f}  |  "
          f"RL profit spread (max-min): {max(f.profit for f in rl_firms) - min(f.profit for f in rl_firms):,.0f}")

print(f"\n{'='*60}")
print(f"  COMPETITIVE SUMMARY ({N_STEPS} steps)")
print(f"{'='*60}")
